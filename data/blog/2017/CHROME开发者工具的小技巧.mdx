...
转载自：[酷 壳 – CoolShell](http://coolshell.cn/articles/17634.html)
...
![代码格式化](http://coolshell.cn//wp-content/uploads/2017/01/pretty-code.gif)

# 强制DOM状态
有些HTML的DOM是有状态的，比如标签，其会有 active，hover， focus，visited这些状态，有时候，我们的CSS会来定关不同状态的样式，在分析网页查看网页上DOM的CSS样式时，我们可以点击CSS样式上的 :hov 这个小按钮来强制这个DOM的状态。

![强制DOM状态](http://coolshell.cn//wp-content/uploads/2017/01/state.gif)
...
![动画](http://coolshell.cn//wp-content/uploads/2017/01/animation.gif)
...
```
于是你就可以直接修改网页上的内容了
> 下面这个抓屏中还演示了一个如何清空console的示例。你可以输入 clear() 或是 按 Ctrl+L（Windows下），CMD + K (Mac下)

![直接编辑网页](http://coolshell.cn//wp-content/uploads/2017/01/editor.gif)

# 网络限速
你可以设置你的网络的访问速度来模拟一个网络很慢的情况。

![网络限速](http://coolshell.cn//wp-content/uploads/2017/01/custom-network-throttling-profiles.gif)
...
![复制HTTP请求](http://coolshell.cn//wp-content/uploads/2017/01/curl.gif)
...
![抓个带手机的图](http://coolshell.cn//wp-content/uploads/2017/01/device.gif)

我抓的图如下（当然，不是所有的手机都有frame的）

![手机图](http://coolshell.cn//wp-content/uploads/2017/01/coolshell.cn-iPhone-6-Plus-1-148x300.png)
...
![给DOM设置断点](http://coolshell.cn//wp-content/uploads/2017/01/break.dom_-768x531.png)

## 给XHR和Event Lisener设置断点
在 Sources 面页中，你可以看到右边的那堆break points中，除了上面我们说的给DOM设置断点，你还可以给XHR和Event Listener设置断点，载图如下：

![给XHR和Event Lisener设置断点](http://coolshell.cn//wp-content/uploads/2017/01/breakpoints-768x943.png)
...
![DOM操作](http://coolshell.cn//wp-content/uploads/2017/01/events-geteventlisteners_expanded-768x267.png)
* 你还可以使用 monitorEvents($("selector")) 来监控相关的事件。比如：
```
monitorEvents(document.body, "click");
```
![monitorEvents](http://coolshell.cn//wp-content/uploads/2017/01/monitor-events-768x283.png)

## Console中的一些函数
### monitor函数
使用 monitor函数来监控一函数，如下面的示例

![monitor函数](http://coolshell.cn//wp-content/uploads/2017/01/monitor-300x112.png)
...
![inspect函数](http://coolshell.cn//wp-content/uploads/2017/01/inspect-768x345.png)
...
```
[左耳朵](http://coolshell.cn//wp-content/uploads/2017/01/console.log_-300x92.png)
...
```
![Console的输出](http://coolshell.cn//wp-content/uploads/2017/01/console.log2_-604x243.png)
...
console.table(pets)
```
[](http://coolshell.cn//wp-content/uploads/2017/01/console.table_-604x258.png)
...
![快捷键](http://coolshell.cn//wp-content/uploads/2017/01/shortcuts-768x350.png)
